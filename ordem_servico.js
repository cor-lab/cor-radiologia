...
function imprimirOS(agId) {
    ...
    itensInfo.forEach(function(info, idx) {
        ...
        var badgeConv = info.foraFat
            ? "<span style='font-size:8px;background:#BA7517;color:#fff;padding:1px 3px;border-radius:2px;margin-left:3px;vertical-align:middle'>CONV.</span>"
            : "";
        // Se há desconto e item é particular, mostrar preço original riscado + preço com desconto
        var precoCelula = "";
        if (temDesconto && !info.foraFat) {
            precoCelula = "<span style='text-decoration:line-through;color:#999;font-size:10px'>R$ " +
    ...
    if (ehMisto) {
        ...
        if (temDesconto) {
            totalHtml += "<tr style='background:#f9f9f9'>" +
                "<td colspan='3' style='padding:2px 6px;text-align:right;font-size:10px;color:#666'>Subtotal Particular</td>" +
                "<td style='padding:2px 6px;text-align:right;font-size:10px;color:#666'>R$ " + totalParticularBruto.toFixed(2).replace(".", ",") + "</td>" +
                "</tr>";
            totalHtml += "<tr style='background:#f9f9f9'>" +
                "<td colspan='3' style='padding:2px 6px;text-align:right;font-size:10px;color:#c00'>Desconto" + (descontoPerc > 0 ? " (" + descontoPerc.toFixed(0) + "%)" : "") + "</td>" +
                "<td style='padding:2px 6px;text-align:right;font-size:10px;color:#c00'>-R$ " + descontoValor.toFixed(2).replace(".", ",") + "</td>" +
                "</tr>";
        }
        totalHtml += "<tr style='background:#e8f5e9'>" +
            "<td colspan='3' style='padding:4px 6px;text-align:right;font-weight:bold;color:#1D9E75;font-size:12px'>💵 PAGO NO CAIXA (Particular)</td>" +
            "<td style='padding:4px 6px;text-align:right;font-weight:bold;color:#1D9E75;font-size:12px'>R$ " + totalParticularComDesc.toFixed(2).replace(".", ",") + "</td>" +
            "</tr>";
        totalHtml += "<tr style='background:#fff8e8'>" +
            "<td colspan='3' style='padding:4px 6px;text-align:right;font-weight:bold;color:#BA7517;font-size:12px'>📋 FATURAR CONVÊNIO</td>" +
            "<td style='padding:4px 6px;text-align:right;font-weight:bold;color:#BA7517;font-size:12px'>R$ " + totalConvenio.toFixed(2).replace(".", ",") + "</td>" +
            "</tr>";
        ...
        if (temDesconto) {
            totalHtml += "<tr style='background:#f9f9f9'>" +
                "<td colspan='3' style='padding:2px 6px;text-align:right;font-size:10px;color:#666'>Subtotal</td>" +
                "<td style='padding:2px 6px;text-align:right;font-size:10px;color:#666'>R$ " + totalParticularBruto.toFixed(2).replace(".", ",") + "</td>" +
                "</tr>";
            totalHtml += "<tr style='background:#f9f9f9'>" +
                "<td colspan='3' style='padding:2px 6px;text-align:right;font-size:10px;color:#c00'>Desconto" + (descontoPerc > 0 ? " (" + descontoPerc.toFixed(0) + "%)" : "") + "</td>" +
    ...
    var avisoMistoBox = "";
    if (ehMisto) {
        avisoMistoBox = "<div style='background:#fff8e8;border:1px solid #BA7517;border-radius:4px;padding:5px 8px;margin:6px 0;font-size:10px;color:#854F0B'>" +
    ...
    var html = "<!DOCTYPE html><html><head><meta charset='UTF-8'>" +
        ...
        ".seq-box .label{font-size:9px;color:#666;text-transform:uppercase;letter-spacing:.8px}" +
        ".seq-box .numero{font-size:22px;font-weight:bold;color:#2d3a6e;line-height:1.1}" +
        ".info-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:3px 14px;margin-bottom:8px;font-size:11px}" +
        ".info-grid .label{font-weight:bold;color:#555;font-size:9px;text-transform:uppercase;letter-spacing:.3px}" +
        ".info-grid .valor{font-size:12px;line-height:1.2}" +
        ".total-row{background:#f0f4ff;font-weight:bold;font-size:13px}" +
        ".barcode{text-align:center;margin:6px 0}" +
        ".barcode svg{height:32px}" +
        ".entrega-box{margin:6px 0;padding:5px 10px;background:#f8f9fa;border:1px solid #dee2e6;border-radius:4px;display:flex;gap:24px;font-size:11px}" +
        ".entrega-box .label{font-weight:bold;color:#555;font-size:9px;text-transform:uppercase}" +
        ".entrega-box .valor{font-size:11px}" +
        ".setores{margin:8px 0 6px}" +
        ".setores-titulo{font-size:10px;font-weight:bold;color:#2d3a6e;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;border-bottom:1.5px solid #2d3a6e;padding-bottom:2px}" +
        ...
        "<span style='font-size:11px;font-family:monospace;letter-spacing:1.5px'>" + seqAtend + "</span></div>" +
...
console.log("[COR] Modulo ordem de servico v2.6 carregado (desconto em mistos + A4 pagina unica)");
